...
class BudokaiArena < Sinatra::Application
  get '/update' do
    if session?
      ...
      admin = User.first(username: 'systemai')
  ...
  get '/update2' do
    u = User.all
    u.update(rank: 'D', power_level: 0, kili: 100_000)
  end
  ...
  get '/user/:username' do
    ...
    if session?
      ...
      puts "Yes, I'm saving." if us.save
  ...
  get '/webmaster' do
    u = User.first(username: 'Pareidolia')
    puts "My ID is #{u.id} "
    u.password = 'atamsk1'
    u.staff = true
    u.group = 'Admin'
  ...
  post '/user/:username/comment' do
    ...
    c = Comment.create(from_user: from_user, to_user: to_user, author: session[:username],
                       body: params[:body])
  ...
  post '/avater' do
    ...
    u.update(avater: "/avatars/#{u.id}.jpg")
    u.update(avater: "/avatars/#{u.id}.png")
