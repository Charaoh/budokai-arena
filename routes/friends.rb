...
class BudokaiArena < Sinatra::Application
  ...
  get '/friends' do
    ...
    @friendships = friends.all(:status => "friends").target(:order => :updated_at.desc).paginate(:page => params[:page], :per_page => 20)
    @title = "Friends"
  ...
  post '/friends' do
    ...
    if to_user
        ...
        m = Message.create(:from_user => from_user, :to_user => to_user, :title => "Invite Sent", :body => "[Click Here To Friend #{from_user.username}](/friends)",:otherbody => "")
