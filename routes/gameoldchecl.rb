...
winning_planet = winning_user.members.first(:status => "Joined").planet
losing_planet = losing_user.members.first(:status => "Joined").planet
...
when 'sBo'
team['sBo'] = Bergamo.new(controlling_player, s[i], stats['sBo'], 170, true, [8, 10, 9, 9],c[i],i,"Bergamo ")
when 'zZn'
team['zZn'] = Zarbon.new(controlling_player, s[i], stats['zZn'], 160, true, [8, 10, 9, 11],c[i],i,"Zarbon Z")
when 'zCl'
team['zCl'] = Cell.new(controlling_player, s[i], stats['zCl'], 150, true, [1, 1, 1, 1],c[i],i,"Cell Z")
when 'zCJr'
team['zCjr'] = CellJr.new(controlling_player, s[i], stats['zCjr'], 120, true, [0, 0, 0, 0],c[i],i,"Cell Jr Z")
when 'zFa'
...
when 'zDe'
team['zDe'] = Dende.new(controlling_player, s[i], stats['zSo'], 160, true, [5, 6, 12, 11],c[i],i,"Dende Z")
end
end
...
your_characters_info['y_c1_transformation'] = y_c1.transformation[1].to_s
your_characters_info['y_c2_transformation'] = y_c2.transformation[1].to_s
your_characters_info['y_c3_transformation'] = y_c3.transformation[1].to_s
...
your_characters_info['o_c1_transformation'] = o_c1.transformation[1].to_s
your_characters_info['o_c2_transformation'] = o_c2.transformation[1].to_s
your_characters_info['o_c3_transformation'] = o_c3.transformation[1].to_s
...
class Character
  attr_accessor :health,:energy,:stunned,:block,:controlling_player,:cooldown,:strength,:ki,:defense,:transformation,:current_defense,:effect,:skillhold,:max,:experience,:counter,:effecting,:speed,:attacked,:reflect,:info,:conditions
  ...
  @skillhold = s
  @transformation = [false, "none", 0, 0, 0, 0, false]#[Transformed T / F, MoveId, Ki, Strength, Defense, Speed, We do you want the pic to change]
  @effect = []
  @effecting = []
  @experience = stats[0]
  @attacked = [-1, -1, -1,-1]
  ...
  @conditions = [0,0,0,0] #0.Dazed  1.Cannot Be killed 2.Increase Cooldown 3.Increase Bp by 1
  ...
  if player.health > 0  && player.conditions[3]
  ...
  def reflect_s(player, name, type, target, turns)# Reflecting Skills Type 1: Self 2: Specific
  if type == 1
  player.reflect.push(name)
  player.reflect.push("protect")
  player.reflect.push(target)
  
  player.counter[6] = turns
  return true
  elsif type == 2
  player.reflect.push(name)
  player.reflect.push("stop")
  player.reflect.push(target)

  player.counter[7] = turns
...
if self.conditions[0] != 0 && type < 4
...
elsif self.counter[7] != 0 && type < 4
  d = self.reflect.index("stop")
  #player = self.reflect[d]
  if self.reflect[d+1] == "Ally"
  if self.info[1] == 0
  a = [1,2].shuffle 
  ...
  elsif self.reflect[d+1] == "Enemy"
  other[i] = self
  else
  other[i] = self.reflect[d+1]
  end
  elsif other[i].counter[6] != 0 && type < 4
  d = other[i].reflect.index("protect")
  other[i]  = other[i].reflect[d+1]
...
if self.conditions[0] != 0 && type < 4
  ...
  other = self.reflect[d+1]
  end
  elsif other.counter[6] != 0 && type < 4
   d = other.reflect.index("protect")
   other  = other.reflect[d+1]
...
if self.counter[0] == 0 && other[i].counter[4] == 0
count += 1
else
success[-1] = "Counter"
reason = 2
...
if self.counter[0] == 0 && self.counter[1] == 0 && other[i].counter[4] == 0
count += 1
...
if self.counter[0] == 0 && self.counter[2] == 0 && other[i].counter[4] == 0
count += 1
else
success[-1] = "Counter"
reason = 2
...
if self.counter[0] == 0 && self.counter[3] == 0 && other[i].counter[4] == 0
count += 1
else
success[-1] = "Counter"
reason = 2
...
if self.counter[0] == 0 && other.counter[4] == 0
count += 1
else
target.push("Counter")
reason = 2
...
if self.counter[0] == 0 && self.counter[1] == 0 && other.counter[4] == 0
count += 1
else
target.push("Counter")
reason = 2
...
if self.counter[0] == 0 && self.counter[2] == 0  && other.counter[4] == 0
count += 1
else
target.push("Counter")
reason = 2
...
skill_turns(self, 99, 100, "10000", 3, true)
logs.push(0,move,c[1].health,[self.controlling_player.vs,self.info[1]],c[2],c[1].info[1]) 
...
when 129
m = "129"
if self.effecting[c+1].attacked[0].kind_of?(Array)
  length = self.effecting[c+1].attacked[0].length
  for i in 0..length
...
if c[0]
skill_turns(c[1], 2, 128, "60", 1, true)
c[1].conditions[0] = 0
c[1].conditions[1] = 0
c[1].conditions[2] = 0
c[1].conditions[3] = 0
...
skill_turns(self, 99, 101, "10015", 3, true)
logs.push(0,move,c[1].health,[self.controlling_player.vs,self.info[1]],c[2],c[1].info[1]) 
end
end
#----------------------------Skill Area---------------------------------------
...
skill_turns(self, 99, 101, "10014", 3, true)
logs.push(0,move,c[1].health,[self.controlling_player.vs,self.info[1]],c[2],c[1].info[1]) 
end
end
#----------------------------Skill Area---------------------------------------
...
c = check(1, 35, skill,count+1, player, 3,logs,[false,false,false])
if c[0]
s_damage(c[1],15)
c[1].stunned[2] = 2
logs.push(0,move,c[1].health,[self.controlling_player.vs,self.info[1]],c[2],c[1].info[1]) 
...
boost = count * 5
p_k_damage(c[1],10+ boost)
if c[1].effect.index("139") != nil
c[1].stunned[1] = 3
end
...
skill_turns(self, 99, 101, "10013", 3, true)
logs.push(0,move,c[1].health,[self.controlling_player.vs,self.info[1]],c[2],c[1].info[1]) 
end
...
when 162 # Evil Comet
m = "162"
if self.effecting[c+1].block[0] == 0 && self.effecting[c+1].block[2] == 0 && self.stunned[0] == 0 && self.stunned[2] == 0 && !stop
s_damage(self.effecting[c + 1], 5)
end
if self.effecting[c+1].attacked[0] != -1
...
when 162 # Evil Comet
c = check(1, 30, skill, 4, player, 1,logs,[false,false,false])
if c[0]
l = c[1].length - 1
i = 0
for i in 0..l
...
when 163 # Parasetic Meda Clones
c = check(0, 15, skill, 0, player, 2,logs,[false,false,false])
if c[0]
...
c[1].energy -= count * 10
self.energy += count * 10
c[1].effect.delete("163")
logs.push(0,move,c[1].health,[self.controlling_player.vs,self.info[1]],c[2],c[1].info[1]) 
else
logs.push(c[3],move,self.health,[self.controlling_player.vs,self.info[1]],c[2],c[1].info[1])  
end
...
heal(self,count*5,0)
   if play[0] != self
   heal(play[0],count*5/2,0)
   end
   
   if play[1] != self
   heal(play[1],count*5/2,0)
   end
   
   if play[2] != self
   heal(play[2],count*5/2,0)
   end
end
health = [player[0].health,player[1].health,player[2].health]
logs.push(4,move,health,[self.controlling_player.vs,self.info[1]],c[3],c[4])  
...
m = "32"
if self.effecting[c] == 1 && !stop
p_k_damage(effecting[c + 1], 20)
elsif self.stunned[2] != 0 && self.stunned[0] != 0 && !stop
p_k_damage(effecting[c + 1], 10)
...
skill_turns(self, 99, 101, "10001", 3, true)
...
skill_turns(self, 99, 100, "10017", 3, true)
logs.push(0,move,c[1].health,[self.controlling_player.vs,self.info[1]],c[2],c[1].info[1]) 
elsif self.transformation[0] == true && c[0]
transformskill(0, 0, 3)
end #Add more transformations before this "end" if there are any others.
...
pl = self.controlling_player.enemy.values
 if pl[0].attacked[0].kind_of?(Array)
   ...
   self.speed += 3
   skill_turns(pl[0],2,124,"120",1,true)
   end
   ...
   self.speed += 3 
   skill_turns(pl[1],2,124,"120",1,true)
   end
   ...
   self.speed += 3
   skill_turns(pl[2],2,124,"120",1,true)
   end
...
skill_turns(self, 99, 101, "10018", 3, true)
...
when 115
m = "115"  
if self.health <= (self.effecting[c] - 100) * 5
self.effecting[c] = 1
self.conditions[1] = 1
stop = true
else
...
skill_turns(self, 99, 100, "10016", 3, true)
logs.push(0,move,c[1].health,[self.controlling_player.vs,self.info[1]],c[2],c[1].info[1]) 
end
...
when 115 # Immortality
if self.transformation[1] == "10016"
...
c = check(3, 25, skill, 0, player, 4,logs,[false,false,false])
if c[0] && self.effect.count("115") == 0
skill_turns(c[1],99,115,"115",1,true)
c[1].conditions[1] = 99
logs.push(0,move,c[1].health,[self.controlling_player.vs,self.info[1]],c[2],c[1].info[1]) 
...
skill_turns(self, 99, 101, "10004", 3, true)
self.transformation[6] = true
self.transformation[0] = true
elsif self.transformation[0] == true && self.stunned[0] == 0 && c[0]
self.transformation[0] = false
...
c = check(2, 20, name[2], 2, name[0], 2,logs,[false,false,false])
if c[0]
skill_turns(c[1], 2, 42, "42", 1, false)
c[1].counter[1] = 2 
end
else
c = check(2, 20, name[2], 3, name[0], 2,logs,[false,false,false])
if c[0]
...
when 44 # Destructo Disk Barrage
m = "44"
if self.stunned[0] == 0 && self.stunned[1] && self.effecting[c + 1].block[0] == 0 && self.effecting[c + 1].block[1] == 0
...
skill_turns(self, 99, 101, "10005", 3, true)
self.transformation[6] = true
...
skill_turns(c[1], 2, 52, "52", 1, true)
c[1].defense -= 3
else
...
if c[0]
if c[1].effect.count("53") == 0
skill_turns(c[1], 2, 54, "54", 2, true)
c[1].speed -= 3
else
...
skill_turns(self, 99, 101, "10007", 3, true)
logs.push(0,move,c[1].health,[self.controlling_player.vs,self.info[1]],c[2],c[1].info[1]) 
elsif self.transformation[0] == true && self.stunned[0] == 0 && self.stunned[3] == 0 && c[0]
transformskill(0, 0, 3)
end 
...
c = check(2,20,skill,0,player,4,logs,[false,false,false])  
if self.transformation[0] == false && c[0]
transformskill([0.6, 0.6, 0.6, 0], "10008", 1)
skill_turns(self, 99, 101, "10008", 3, true)
...
when 75 # Aerial Smash
c = check(1, 25, skill, 2, player, 2,logs,[false,false,false])
if c[0]
s_damage(c[1],15)
self.speed += 3
logs.push(0,move,c[1].health,[self.controlling_player.vs,self.info[1]],c[2],c[1].info[1]) 
...
when 94 # Self Storm
c = check(1, 30, name[2], 3, name[0], 3,logs,[false,false,false])
if c[0]
...
c = check(4,50,4,4,player,0,logs,[false,false,false])
if c[0]
...
self.health = 0
stop = true
power_up(self.effecting[c+1],[20,20,20,20])
end
end
effecting[c] -= 1
if (effecting[c] == 0 || stop)
...
c = check(2,40,skill,1,player,4,logs,[false,false,false])
if transformation[0] == false && player.info[0] == "zPo" && c[0] 
transformation[0] = true
skill_turns(player, 99, 101, "10006", 3, false)
logs.push(0,move,c[1].health,[self.controlling_player.vs,self.info[1]],c[2],c[1].info[1]) 
end 
end
...
c = check(3, 50, skill, 3, player, 1,logs,[false,false,false])
if c[0]
boost = c[1].effect.count('60')
damage = 40 + 15 * boost
selfdamage = 10 + 10 * boost
...
boost = c[1].effect.count('60')
damage = 20 + 5 * boost
selfdamage = 5 + 5 * boost
self.health -= selfdamage
p_k_damage(c[1],damage)
logs.push(0,move,c[1].health,[self.controlling_player.vs,self.info[1]],c[2],c[1].info[1])
else
logs.push(c[3],move,self.health,[self.controlling_player.vs,self.info[1]],c[2],c[1].info[1])  
end
when 60
c = check(1, 20, skill, 2, player, 3,logs,[false,false,false])
if c[0]
c[1].block[4] = 2
c[1].stunned[3] = 2
skill_turns(c[1], 99, 60, "60", 1, true)
...
when 99
m = "102"
if self.effecting[c+1] != self && !stop
self.effecting[c+1].energy -= 15
self.health -= 5
else
self.effecting[c] = 0;
...
c[1].stunned[3] = 3
skill_turns(c[1], 3, 61, "61", 3, true)
logs.push(0,move,c[1].health,[self.controlling_player.vs,self.info[1]],c[2],c[1].info[1]) 
else
logs.push(c[3],move,self.health,[self.controlling_player.vs,self.info[1]],c[2],c[1].info[1])
end
...
when 63
c = check(3, 50, skill, 5, player, 3,logs,[false,false,false])
if c[0]
reflect_s(c[1], "63", 2, "Ally", 3)
...
when 69
c = check(2, 30, name[2], 3, name[0], 3,logs,[false,false,false])
if c[0]
bpeffect(c[1],-1)
c[1].counter[1] = 2
skill_turns(c[1], 2, 69, "69", 3, false)
end 
end
end
...
when 69
m = "69"
if self.effecting[c+1].attacked[1] != -1
percent_up(self.effecting[c+1],[-0.25,-0.25,0,0])
else
...
when 50 # Double Sunday
m = "50"
if self.stunned[0] == 0 && self.stunned[1] == 0 && self.effecting[c+1].block[0] == 0 && self.effecting[c+1].block[1] == 0 && self.counter[0] == 0 && self.counter[1] == 0 && self.counter[4] == 0 && self.counter[5] == 0
p_k_damage(self.effecting[c+1],10)
end
...
if find == nil
skill_turns(c[1],3,50,"50",2,true)
else
k_damage(player, 30)
skill_turns(c[1],4,50,"50",2,true)
end
logs.push(0,move,c[1].health,[self.controlling_player.vs,self.info[1]],c[2],c[1].info[1]) 
else
logs.push(c[3],move,self.health,[self.controlling_player.vs,self.info[1]],c[2],c[1].info[1])
...
if self.transformation[7] > self.health
damage = self.transformation[7] - self.health 
if damage > 0
self.effecting[c+1].health -= damage
end
end
self.transformation[7] = self.health
...
if c[0]
skill_turns(player, 99, 101, "10009", 3, true)
...
when 57 #Self Destruct
if self.transformation[1] == "10009"
c = check(2, 40, skill, 3, player, 2,logs,[false,false,false])
if c[0] 
skill_turns(player,99,58,"57",3,false)
end 
else
c = check(2, 40, skill, 3, player, 2,logs,[false,false,false])
if c[0]
skill_turns(player,99,57,"57",3,false)
end 
end
end
end
